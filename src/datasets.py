...
def read_labeled_tsv(path: str | Path) -> pd.DataFrame:
    """Read a labeled TSV file and keep the first text column after label."""
    try:
        data = pd.read_csv(path, sep="\t", header=None, dtype="string", on_bad_lines="skip")
    ...
    if data.shape[1] < 2:
        raise ValueError("A labeled TSV dataset must contain at least label and text columns.")
